...
func createTables() error {
	stmts := []string{
		...
		// Shop ratings: aggregate buyer scores for a shop (店铺评分).
		`CREATE TABLE IF NOT EXISTS shop_ratings (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			shop TEXT NOT NULL,
			user_id INTEGER NOT NULL,
			description_score INTEGER NOT NULL DEFAULT 5,
			logistics_score INTEGER NOT NULL DEFAULT 5,
		...
		// Order invoices: an electronic invoice requested for a paid order (发票).
		...
		// Price alerts: users who want to be notified when a product drops to a target price (降价提醒).
